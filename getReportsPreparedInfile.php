<?php
/**
 * Get reports for all  users. We use LOAD data in file
...
//WHY fields keyword and matchType ??
    ...
    writeLog('getResponsePreparedInfile', 'Get report ' . $reportData['type']);
		...
		//	continue;
            do {
                ...
                if ($reportResponse['success'] == true AND $reportResponse['code'] == 200 AND (!isset($reportResponse['response']['0']))) {
                    ...
                    writeLog('getResponsePreparedInfile', 'we got empty report with code 200  ');
                }
            ...
            if (!isset($reportResponse['response']['0'])) {
                ...
                writeLog('getResponsePreparedInfile', 'we CANT GET report in ' . $attemps . ' attempts  ');
            } else {
                echo "\033[32m WE get report :) \033[0m  \n";
                writeLog('getResponsePreparedInfile', 'we get report in ' . $attemps . ' attempts  ');
            }
            if ($reportResponse['success'] AND isset($reportResponse['response']['0'])) {
                writeLog('getResponsePreparedInfile', 'we need insert  ' . count($reportResponse['response']) . '  records  ');
                ...
                //execute file
                writeLog('getResponsePreparedInfile', 'Try load from file  ');
                ...
                if (unlink("reports/" . $reportResponse['requestId'] . "/ADSReport.csv")) {
                    ...
                    if (rmdir($dir)) {
                        ...
                        writeLog('getResponsePreparedInfile', 'File and folder deleted  ');
                    } else {
                        echo "\033[31m Cant remove folder  \033[0m  \n";
                        writeLog('getResponsePreparedInfile', 'Cant remove folder  ');
                    }
                } else {
                    echo "\033[31m Cant remove file  \033[0m  \n";
                    writeLog('getResponsePreparedInfile', 'Cant remove file   ');
                }
    ...
    try {
        do {
            ...
            if (!$r) {
                ...
                writeLog('getResponsePreparedInfile', " MYSQL error  " . $error[0] . "  " . $error[2]);
                echo "\033[31m Sleep ....  \033[0m  \n";
                sleep(5);
  //              exit();
            } else {
                writeLog('getResponsePreparedInfile', "  processed " . $r . " rows  ");
        ...
        writeLog('getResponsePreparedInfile', " Exeption  " . $e->getMessage());
        exit();
    }
}


writeLog('getResponsePreparedInfile', ' --FINISH--  ');
echo 'getResponsePreparedInfile   FINISHED';
